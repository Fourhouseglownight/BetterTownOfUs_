...
namespace BetterTownOfUs.Roles
{
    public class Prophet : Role
    {
        ...
        public void Revelation()
        {
            ...
            if (target == null)
            {
                PluginSingleton<BetterTownOfUs>.Instance.Log.LogMessage(
                    $"The Prophet has no more eligible revelations to receive.");
                return;
            }

            PluginSingleton<BetterTownOfUs>.Instance.Log.LogMessage($"The Prophet has received information that {target.name} is a Crewmate role. "
