...
namespace BetterTownOfUs.Roles
{
    public interface IMeetingGuesser
    {
        ...
        public bool CanMissKill();
        public bool MissingGuess();

        private static Sprite CycleSprite => BetterTownOfUs.CycleSprite;
        ...
        private static Action Guess(IMeetingGuesser role, PlayerVoteArea voteArea, Action<PlayerControl, Role> doOnCorrectGuess)
        {
            void Listener()
            {
                ...
                PlayerControl guesser = role.Player;

                if (CustomGameOptions.GuessSnitchViaCrewmate && actualRole.RoleType == RoleEnum.Snitch && currentGuess == RoleEnum.Crewmate)
                    currentGuess = RoleEnum.Snitch;

                if (actualRole.RoleType != currentGuess)
                {
                    if (CustomGameOptions.GuesserMissKill && role.CanMissKill())
                    {
                        role.MissingGuess();
                        GuesserKill.RpcMissKill(guesser);
                        return;
                    }
                    if (guesser == ParasiteShift.Parasitized)
                    {
                        ShowHideButtons.HideButtons(role);
                    }

                    KillFromMeetingGuess(role, guesser, actualRole);
        ...
        {
            GuesserKill.RpcMurderPlayer(player);
    ...
    public class ShowHideButtons
    {
        ...
        )
        {
            if (
                ...
                !CustomGameOptions.GuesserMultiKill
            )
            {
                HideButtons(role);
                return;
        ...
        {
            if (!CustomGameOptions.SGAfterVote && Role.GetRole(PlayerControl.LocalPlayer) is IMeetingGuesser role)
            {
                HideButtons(role);
            }
        }
